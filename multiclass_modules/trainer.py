...
class BaseTrainer(object):
    def __init__(self, model, criterion, metric_ftns, optimizer, args):
        ...
        # self.model = DDP(model, device_ids=device_ids, output_device=args.local_rank)
        if len(device_ids) > 1:
    ...
    def _print_best_to_file(self):
        ...
        record_table = record_table.append(
            self.best_recorder["val"], ignore_index=True)
        record_table = record_table.append(
            self.best_recorder["test"], ignore_index=True
        )
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (
            ...
            output, images_feature = self.model(
                images, reports_ids, mode="train")
            caption_loss, class_loss, loss = self.criterion(
                output,
                reports_ids,
                reports_masks,
                images_feature,
            ...
            global global_step
            global_step += 1
            images_feature = torch.sigmoid(images_feature).data
        ...
        writer.add_scalars(
            "train_loss",
            {
                ...
                "cls_loss": cls_loss / len(self.train_dataloader),
                "cap_loss": cap_loss / len(self.train_dataloader),
            },
            epoch,
        )
        writer.add_scalars(
        ...
        with torch.no_grad():
            ...
            for batch_idx, (
                ...
                images, reports_ids, reports_masks, labels = (
                    images.to(self.device),
                    reports_ids.to(self.device),
                    reports_masks.to(self.device),
                    labels.to(self.device),
                )
                output, images_feature = self.model(images, mode="sample")
