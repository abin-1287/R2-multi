...
class Transformer(nn.Module):
    ...
    def decode(self, hidden_states, src_mask, tgt, tgt_mask):
        ...
        return self.decoder(tgt, hidden_states, src_mask, tgt_mask)
...
class EncoderDecoder(AttModel):
    ...
    def make_model(self, tgt_vocab):
        ...
        model = Transformer(
            ...
            # rm)
            Decoder(DecoderLayer(self.d_model, c(attn), c(attn), 
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        ...
        return fc_feats[..., :1], att_feats[..., :1], att_feats, att_masks
    ...
    def _forward(self, images, seq, att_masks=None):
        # iu_xray
        # fc_feats, att_feats = self.cocamodel._encode_image(images[:, 0])
        # fc_feats, att_feats = self.cocamodel._encode_image(images[:, 1])
        # fc_feats = torch.cat((fc_feats_0, fc_feats_1), dim=1)
        # att_feats = torch.cat((att_feats_0, att_feats_1), dim=1)
        # mimic
        fc_feats, att_feats = self.cocamodel._encode_image(images)
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq)
        text_latent, token_embs = self.cocamodel._encode_text(seq)
        out = self.model(att_feats, token_embs, att_masks, seq_mask)
        outputs = F.log_softmax(self.logit(out), dim=-1)
        return outputs,fc_feats,text_latent
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        ...
        _, token_embs = self.cocamodel._encode_text(ys)
        out = self.model.decode(memory, mask, token_embs, subsequent_mask(ys.size(1)).to(memory.device))
